...
final class Crawler implements CrawlerInterface, LoggerAwareInterface
{
    ...
    public function initialize($crawlUrlProviders) : int
    {
        ...
        foreach ($crawlUrlProviders as $crawlUrlProvider) {
            $this->logger->info(\sprintf("%s: enqueueing crawl URLs", \get_class($crawlUrlProvider)));
            $numEnqueued = $this->enqueueProvidedCrawlUrls($crawlUrlProvider);
            $this->logger->info(\sprintf("%s: %d crawl URLs enqueued", \get_class($crawlUrlProvider), $numEnqueued));
    ...
    private function enqueueProvidedCrawlUrls(CrawlUrlProviderInterface $crawlUrlProvider) : int
    {
        ...
        foreach ($crawlUrlProvider->provide() as $crawlUrl) {
            ...
            if ($numEnqueued < 10) {
                $this->logger->debug(\sprintf("%s: enqueueing %s", \get_class($crawlUrlProvider), $crawlUrl->url()));
            } elseif (++$numMore >= 50) {
                $this->logger->debug(\sprintf("%s: enqueued %d more URLs...", \get_class($crawlUrlProvider), $numMore));
                $numMore = 0;
            }
